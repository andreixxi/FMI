...
class Nod:
    def __init__(self, info, h = None):
        ...
        if h is not None:
            ...
            self.h = (info[1][0] + info[1][1]) // 2 #int(media dintre mis si can de pe malul DREPT)
...
class Problema:
    def __init__(self):
        self.N = 3 #nr canibali, nr misionari
        self.M = 2 #nr locuri in barca
        
        # misionari, canibali pe malul stang/ drept  # de tip Nod
        # al 3-lea element e malul pe care e barca
        self.nod_start = Nod([[0, 0], [3, 3], 1])
        self.nod_scop = [[3, 3], [0, 0], 0]  
...
class NodParcurgere:
    ...
    def expandeaza(self):
        ...
        mal = info[2] # malul pe care se afla barca
        M = self.problema.M 
        
        for misionar in range(min(info[mal][0], M) + 1): 
            
            if misionar == 0:
                c_max = M
            else:
                c_max = min(M - misionar, misionar, info[mal][1])
                
            for canibal in range(c_max + 1):
                if misionar + canibal == 0:
                    continue
                
                succ = [[], [], 1 - mal] #schimb malul
                succ[mal] = [info[mal][0] - misionar, info[mal][1] - canibal] #scad
                succ[1 - mal] = [info[1 - mal][0] + misionar, info[1 - mal][1] + canibal] #adun
                
                # nu se mananca
#                 (mis >= can >= 0 SAU mis = 0) pt fiecare mal
                if (succ[0][0] >= succ[0][1] >= 0 or succ[0][0] == 0)                 and (succ[1][0] >= succ[1][1] >= 0 or succ[1][0] == 0):
